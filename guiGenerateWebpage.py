...
class PageTwo(Frame):
	...
	dictionary = dict(zip(self.treeTranslate.get_children(), self.treeTable.get_children()))
...
#	print 'classname',classname
	...
	for sc in scoreMatching:
		...
		attributeMatch.append(dictionary[sc[1]])
		#self.page4.markup.insert(END,'"'+sc[2]+'":"#####",\n')
	print 'match ',attributeMatch
...
class PageFive(Frame):
	...
	kolumn= str(attributeMatch).strip("[,]")
	kolumn=kolumn.replace("'", "")
	print "select "+kolumn+" from "+tablename+"where "+str(self.treeData['column'][0])+"='"+str(self.treeData.item(selected, "values")[0])+"';"
	
	data=list(preprop.get_tables('uns',"select "+kolumn+" from "+tablename+" where "+str(self.treeData['column'][0])+"='"+str(self.treeData.item(selected, "values")[0])+"';"))
	help.append(self.treeData.item(selected)['text'])
	help+= self.treeData.item(selected)['values']
	print data,properties
	#help.pop(len(help)-1)
	self.markup.insert(END,wordnet1.sudoMarkup(properties,expectedType,classname,list(data[0])))
